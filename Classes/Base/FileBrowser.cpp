...
/*
* Get filenames we selected from filebrowser.
*		If we selected multiple files then the first index contains the directory path
*		and the rest of indices contain only the filename
*		If we selected only one file then the filename contains the full path
*/
std::vector<std::string> FileBrowser::getFileNamesWithoutFolder() const
{
	std::vector<std::string> resVec;
	std::wstring_convert<std::codecvt_utf8_utf16<wchar_t>, wchar_t> converter;
	...
	while (*str)
	{
		std::wstring filename = str;
		str += (filename.length() + 1);
		resVec.push_back(converter.to_bytes(filename));
	}

	// Get rid of directory if we selected multiple files
	// Otherwise remove directory path part from the filename
	if (resVec.size() > 1)
	{
		resVec.erase(resVec.begin());
	}
	...
	{
		resVec[0] = resVec[0].substr(resVec[0].rfind("\\") + 1);
	}
