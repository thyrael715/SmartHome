...
bool Mp3::openFromFile(const std::string& filename)
{
	...
	auto frameNum = mpg123_framelength(myHandle);
	auto frameDurationInSec = mpg123_tpf(myHandle);

	myDuration = sf::microseconds((sf::Int32)(frameNum * frameDurationInSec * 1000000));
...
void Mp3::onSeek(sf::Time timeOffset)
{
	...
	if (myHandle)
	{
		off_t sampleOffset = (size_t)timeOffset.asSeconds() * this->getSampleRate();
		mpg123_seek(myHandle, sampleOffset, SEEK_SET);
	}
}
